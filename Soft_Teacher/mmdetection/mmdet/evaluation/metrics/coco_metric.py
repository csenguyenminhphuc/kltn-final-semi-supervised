...
class CocoMetric(BaseMetric):
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        """
        # Debug: print first batch img_ids
        if len(self.results) == 0 and len(data_samples) > 0:
            img_ids = [ds['img_id'] for ds in data_samples]
            print(f"[DEBUG] First batch img_ids ({len(img_ids)}): {img_ids[:8]}")
            print(f"[DEBUG] img_id types: {[type(x).__name__ for x in img_ids[:3]]}")
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        # Debug: check img_ids in predictions
        pred_img_ids = [p['img_id'] for p in preds]
        print(f"[DEBUG] Total predictions: {len(pred_img_ids)}")
        print(f"[DEBUG] Unique pred img_ids: {len(set(pred_img_ids))}")
        print(f"[DEBUG] First 10 pred img_ids: {pred_img_ids[:10]}")
        print(f"[DEBUG] COCO has img_ids: 0-{max(self._coco_api.get_img_ids())}")
        
        # Check if any pred img_id not in COCO
        coco_img_ids = set(self._coco_api.get_img_ids())
        invalid = [x for x in pred_img_ids if x not in coco_img_ids]
        if invalid:
            print(f"[ERROR] {len(invalid)} pred img_ids NOT in COCO: {invalid[:10]}")
        else:
            print(f"[DEBUG] All pred img_ids are valid!")
