...
class DetDataPreprocessor(ImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        if data_samples is not None:
            ...
            # support inputs shape: list[Tensors] or Tensor (B,V,C,H,W) or (N,C,H,W)
            if isinstance(inputs, torch.Tensor) and inputs.dim() == 5:
                # inputs: (B, V, C, H, W) -> per-view pad shape for meta
                B, V = inputs.shape[0], inputs.shape[1]
                batch_input_shape = tuple(inputs[0, 0].size()[-2:])
                # data_samples expected nested list: list of B elements each is list of V DetDataSample
                if isinstance(data_samples, list) and len(data_samples) == B and all(isinstance(x, list) for x in data_samples):
                    for b_idx in range(B):
                        for v_idx in range(V):
                            ds = data_samples[b_idx][v_idx]
                            pad_shape = batch_pad_shape[b_idx * V + v_idx] if isinstance(batch_pad_shape, list) else batch_pad_shape[b_idx]
                            ds.set_metainfo({
                                'batch_input_shape': batch_input_shape,
                                'pad_shape': pad_shape
                            })
                else:
                    # fallback: flat list of length B*V
                    flat_ds = data_samples
                    for idx, ds in enumerate(flat_ds):
                        pad_shape = batch_pad_shape[idx] if isinstance(batch_pad_shape, list) else batch_pad_shape[0]
                        ds.set_metainfo({'batch_input_shape': batch_input_shape, 'pad_shape': pad_shape})
            else:
    ...
    def _get_pad_shape(self, data: dict) -> List[tuple]:
        ...
        # Process data with `default_collate`.
            ...
            # support single-image batch (N, C, H, W) or multi-view batch (B, V, C, H, W)
            if _batch_inputs.dim() == 4:
            ...
            elif _batch_inputs.dim() == 5:
                B, V = _batch_inputs.shape[0], _batch_inputs.shape[1]
                batch_pad_shape = []
                for b in range(B):
                    for v in range(V):
                        ori_input = _batch_inputs[b, v]
                        pad_h = int(
                            np.ceil(ori_input.shape[1] /
                                    self.pad_size_divisor)) * self.pad_size_divisor
                        pad_w = int(
                ...
                raise AssertionError(
                    'The input of `DetDataPreprocessor` should be a NCHW tensor '
...
class MultiBranchDataPreprocessor(BaseDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        # Process both training and non-training (validation) paths the
        # same way: group branches and call the underlying
        # DetDataPreprocessor for each branch. Returning early for
        # non-training caused the downstream `ImgDataPreprocessor` to
        # receive a dict-of-lists in `data['inputs']`, which it does not
        # expect (it expects a list of tensors or a single tensor).

        # Defensive handling: if `data` is not the expected multi-branch
        # dict-of-dicts (for example, a simple collated batch where
        # `data['inputs']` is already a list or tensor), delegate to the
        # underlying `DetDataPreprocessor`. This avoids treating lists as
        # mappings and raising attribute errors during validation.
        if not isinstance(data, dict):
            return self.data_preprocessor(data, training)

        # If inputs/data_samples are already single-branch (list or tensor),
        # pass through to the inner preprocessor.
        if 'inputs' in data and not isinstance(data['inputs'], dict):
            return self.data_preprocessor(data, training)

        # Now we expect data to be a dict mapping keys (inputs/data_samples)
        # to branch dicts. Filter out None entries per-branch.
        for key in data.keys():
            if not isinstance(data[key], dict):
                # Unexpected shape; fall back to inner preprocessor.
                return self.data_preprocessor(data, training)
        ...
        # Normalize outputs: ensure 'data_samples' entries are lists (not None)
        if 'data_samples' in format_data:
            for branch, val in list(format_data['data_samples'].items()):
                if val is None:
                    format_data['data_samples'][branch] = []

        # If we're in evaluation/prediction mode, collapse the
        # multi-branch outputs into a single-branch input that the
        # standard detector predict/forward path expects. Prefer the
        # supervised branch if available, otherwise use unsup_teacher,
        # unsup_student, or the first available branch.
        if not training:
            inputs_map = format_data.get('inputs', {})
            data_map = format_data.get('data_samples', {})
            if isinstance(inputs_map, dict) and len(inputs_map) > 0:
                for pref in ('sup', 'unsup_teacher', 'unsup_student'):
                    if pref in inputs_map:
                        branch = pref
                        break
                else:
                    branch = next(iter(inputs_map.keys()))
                sel_inputs = inputs_map.get(branch)
                sel_data = data_map.get(branch, None)
                # Ensure each DetDataSample has a scale_factor for ROI heads
                if sel_data is not None and isinstance(sel_data, list):
                    for ds in sel_data:
                        try:
                            cur = ds.metainfo if hasattr(ds, 'metainfo') else {}
                        except Exception:
                            cur = {}
                        if 'scale_factor' not in cur:
                            ds.set_metainfo({**cur, 'scale_factor': (1.0, 1.0)})
                return {'inputs': sel_inputs, 'data_samples': sel_data}
