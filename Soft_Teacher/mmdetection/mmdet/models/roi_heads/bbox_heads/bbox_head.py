        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                if self.custom_activation:
                    ...
                    # Debug: Log FG vs BG statistics
                    bg_class_ind = self.num_classes
                    fg_inds = (labels >= 0) & (labels < bg_class_ind)
                    bg_inds = labels == bg_class_ind
                    
                    # Count predictions
                    pred_labels = cls_score.argmax(dim=1)
                    pred_fg = (pred_labels < bg_class_ind).sum().item()
                    pred_bg = (pred_labels == bg_class_ind).sum().item()
                    
                    # Count GT labels
                    gt_fg = fg_inds.sum().item()
                    gt_bg = bg_inds.sum().item()
                    
                    # Store counts for logging
                    losses['num_pred_fg'] = torch.tensor(pred_fg, device=cls_score.device, dtype=torch.float32)
                    losses['num_pred_bg'] = torch.tensor(pred_bg, device=cls_score.device, dtype=torch.float32)
                    losses['num_gt_fg'] = torch.tensor(gt_fg, device=cls_score.device, dtype=torch.float32)
                    losses['num_gt_bg'] = torch.tensor(gt_bg, device=cls_score.device, dtype=torch.float32)
                    
                    if fg_inds.any():
                        fg_acc = accuracy(cls_score[fg_inds], labels[fg_inds])
                        losses['acc_fg'] = fg_acc
                    if bg_inds.any():
                        bg_acc = accuracy(cls_score[bg_inds], labels[bg_inds])
                        losses['acc_bg'] = bg_acc
