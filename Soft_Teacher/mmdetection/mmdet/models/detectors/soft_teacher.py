...
class SoftTeacher(SemiBaseDetector):
    ...
    def rcnn_cls_loss_by_pseudo_instances(self, x: Tuple[Tensor],
        ...
        # Nếu không có mẫu hợp lệ, bbox_head có thể không trả về 'loss_cls'
        if 'loss_cls' not in losses:
            # Tạo một zero-loss đúng device/dtype để tránh KeyError
            zero = bbox_results['cls_score'].sum() * 0
            losses['loss_cls'] = zero

        # Chuẩn hóa theo tổng trọng số (label_weights)
        lw = cls_reg_targets[1]  # label_weights (Tensor)
        # số phần tử (luôn >0), nhưng tổng có thể =0 nếu không có mẫu dương/âm hợp lệ
        denom = lw.sum()
        scale = lw.numel() / denom.clamp(min=lw.new_tensor(1.0))
        losses['loss_cls'] = losses['loss_cls'] * scale
        return losses
