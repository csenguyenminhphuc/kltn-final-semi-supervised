...
class TwoStageDetector(BaseDetector):
    ...
    def __init__(self,
        ...
        # If backbone config is a dict and contains our custom flag 'multi_view',
        # pop it before building so it's not passed to the backbone constructor.
        mv_flag = False
        if isinstance(backbone, dict) and backbone.get('multi_view', False):
            mv_flag = True
            backbone = backbone.copy()
            backbone.pop('multi_view', None)

        self.backbone = MODELS.build(backbone)

        # Wrap with MultiViewBackbone if requested
        if mv_flag:
            try:
                self.backbone = MultiViewBackbone(self.backbone)
            except Exception:
                pass
